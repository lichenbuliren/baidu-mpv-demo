...
;(function () {
  ...
  HeatmapOverlay.prototype.construct = function () {
    ...
    this.changeHandler = qq.maps.event.addListener(map, 'bounds_changed', function () {
      self.draw()
    })
  ...
  HeatmapOverlay.prototype.resize = function () {
    ...
    var div = this.map.getContainer(), width = div.clientWidth, height = div.clientHeight
  ...
  HeatmapOverlay.prototype.update = function () {
    var projection = this.map.getProjection(), zoom, scale, bounds, topLeft
    ...
    topLeft = new qq.maps.LatLng(bounds.getNorthEast().getLat(), bounds.getSouthWest().getLng())
