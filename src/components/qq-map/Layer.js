...
class Layer extends BaseLayer {
  ...
  bindEvent (e) {
    ...
    if (this.options.methods) {
      if (this.options.methods.click) {
        ...
        qq.maps.event.addListener(this.map, 'click', this.clickEvent)
        // map.addEventListener('click', this.clickEvent);
      }
      if (this.options.methods.mousemove) {
        qq.maps.event.addListener(this.map, 'mousemove', this.mousemoveEvent)
  ...
  unbindEvent (e) {
    ...
    if (this.options.methods) {
      if (this.options.methods.click) {
        qq.maps.event.removeListener('click', this.clickEvent);
      }
      if (this.options.methods.mousemove) {
        qq.maps.event.removeListener('mousemove', this.mousemoveEvent);
      }
    }
  }
  ...
  transferToMercator () {
    ...
    if (this.options.coordType !== 'bd09mc') {
      ...
      data = this.dataSet.transferCoordinate(data, function (coordinates) {
        var pixel = projection.fromLatLngToPoint(new qq.maps.LatLng(coordinates[1], coordinates[0]));
  ...
  _canvasUpdate (time) {
    ...
    var map = this.canvasLayer.map;

    var zoomUnit = Math.pow(2, 18 - map.getZoom());
    var projection = map.getProjection();

    var mcCenter = projection.fromLatLngToPoint(map.getCenter());
    var nwMc = new Pixel(mcCenter.x - (getMapSize(map).width / 2) * zoomUnit, mcCenter.y + (getMapSize(map).height / 2) * zoomUnit); //左上角墨卡托坐标
    ...
    var nwPixel = map.getProjection().fromLatLngToPoint(new qq.maps.LatLng(0, 0));
